...
[ApiController]
[Route("api/activity/cat-led")]
[Authorize]
[Produces("application/json")]
public sealed class CatLedController : ControllerBase
{
    private readonly ApplicationDbContext      _db;
    private readonly ILogger<CatLedController> _logger;
    public CatLedController(ApplicationDbContext db, ILogger<CatLedController> logger) { _db = db; _logger = logger; }
    ...
    {
        var q = _db.CatLeds.AsNoTracking();
        ...
        return Ok(PagedResponse<CatLed>.Ok(data, page, pageSize, total));
    }

    [HttpGet("{id:int}")]
    public async Task<IActionResult> GetById(int id, CancellationToken ct)
    {
        var e = await _db.CatLeds.FindAsync([id], ct);
        return e is null ? NotFound(ApiResponse.Fail($"Cat LED {id} not found.")) : Ok(ApiResponse<CatLed>.Ok(e));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CatSelDto dto, CancellationToken ct)
    {
        ...
        var entity = new CatLed { CatSel = dto.CatSel, IsActive = true, CreatedAt = DateTime.UtcNow };
        _db.CatLeds.Add(entity); await _db.SaveChangesAsync(ct);
        return CreatedAtAction(nameof(GetById), new { id = entity.ClId }, ApiResponse<CatLed>.Ok(entity, "Created."));
    }
    ...
    {
        if (!ModelState.IsValid) return BadRequest(ApiResponse.Fail(ModelState.Values.SelectMany(v => v.Errors.Select(e => e.ErrorMessage))));
        var entity = await _db.CatLeds.FindAsync([id], ct);
        if (entity is null) return NotFound(ApiResponse.Fail($"Cat LED {id} not found."));
        entity.CatSel = dto.CatSel; await _db.SaveChangesAsync(ct);
        return Ok(ApiResponse<CatLed>.Ok(entity, "Updated."));
    }
    ...
    public async Task<IActionResult> ToggleStatus(int id, CancellationToken ct)
    {
        ...
        return Ok(ApiResponse.Ok($"Cat LED {id} is now {(entity.IsActive ? "active" : "inactive")}."));
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        _logger.LogWarning("CatLed {Id} soft-deleted", id); return Ok(ApiResponse.Ok("Deleted."));
    }
}
