...
[ApiController]
[Route("api/activity/cat-bottles")]
[Authorize]
[Produces("application/json")]
public sealed class CatBottlesController : ControllerBase
{
    private readonly ApplicationDbContext          _db;
    private readonly ILogger<CatBottlesController> _logger;
    public CatBottlesController(ApplicationDbContext db, ILogger<CatBottlesController> logger) { _db = db; _logger = logger; }
    ...
    {
        var q = _db.CatBottles.AsNoTracking();
        ...
        return Ok(PagedResponse<CatBottle>.Ok(data, page, pageSize, total));
    }

    [HttpGet("{id:int}")]
    public async Task<IActionResult> GetById(int id, CancellationToken ct)
    {
        var e = await _db.CatBottles.FindAsync([id], ct);
        return e is null ? NotFound(ApiResponse.Fail($"Cat bottle {id} not found.")) : Ok(ApiResponse<CatBottle>.Ok(e));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CatSelDto dto, CancellationToken ct)
    {
        ...
        var entity = new CatBottle { CatSel = dto.CatSel, IsActive = true, CreatedAt = DateTime.UtcNow };
        _db.CatBottles.Add(entity); await _db.SaveChangesAsync(ct);
        return CreatedAtAction(nameof(GetById), new { id = entity.CbId }, ApiResponse<CatBottle>.Ok(entity, "Created."));
    }
    ...
    {
        if (!ModelState.IsValid) return BadRequest(ApiResponse.Fail(ModelState.Values.SelectMany(v => v.Errors.Select(e => e.ErrorMessage))));
        var entity = await _db.CatBottles.FindAsync([id], ct);
        if (entity is null) return NotFound(ApiResponse.Fail($"Cat bottle {id} not found."));
        entity.CatSel = dto.CatSel; await _db.SaveChangesAsync(ct);
        return Ok(ApiResponse<CatBottle>.Ok(entity, "Updated."));
    }
    ...
    public async Task<IActionResult> ToggleStatus(int id, CancellationToken ct)
    {
        ...
        return Ok(ApiResponse.Ok($"Cat bottle {id} is now {(entity.IsActive ? "active" : "inactive")}."));
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        _logger.LogWarning("CatBottle {Id} soft-deleted", id); return Ok(ApiResponse.Ok("Deleted."));
    }
}
