...
[ApiController]
[Route("api/activity/cat-colors")]
[Authorize]
[Produces("application/json")]
public sealed class CatColorsController : ControllerBase
{
    private readonly ApplicationDbContext          _db;
    private readonly ILogger<CatColorsController> _logger;
    public CatColorsController(ApplicationDbContext db, ILogger<CatColorsController> logger) { _db = db; _logger = logger; }
    ...
    {
        var q = _db.CatColors.AsNoTracking();
        ...
        return Ok(PagedResponse<CatColor>.Ok(data, page, pageSize, total));
    }

    [HttpGet("{id:int}")]
    public async Task<IActionResult> GetById(int id, CancellationToken ct)
    {
        var e = await _db.CatColors.FindAsync([id], ct);
        return e is null ? NotFound(ApiResponse.Fail($"Cat color {id} not found.")) : Ok(ApiResponse<CatColor>.Ok(e));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CatSelDto dto, CancellationToken ct)
    {
        ...
        var entity = new CatColor { CatSel = dto.CatSel, IsActive = true, CreatedAt = DateTime.UtcNow };
        _db.CatColors.Add(entity); await _db.SaveChangesAsync(ct);
        return CreatedAtAction(nameof(GetById), new { id = entity.CcId }, ApiResponse<CatColor>.Ok(entity, "Created."));
    }
    ...
    {
        if (!ModelState.IsValid) return BadRequest(ApiResponse.Fail(ModelState.Values.SelectMany(v => v.Errors.Select(e => e.ErrorMessage))));
        var entity = await _db.CatColors.FindAsync([id], ct);
        if (entity is null) return NotFound(ApiResponse.Fail($"Cat color {id} not found."));
        entity.CatSel = dto.CatSel; await _db.SaveChangesAsync(ct);
        return Ok(ApiResponse<CatColor>.Ok(entity, "Updated."));
    }
    ...
    public async Task<IActionResult> ToggleStatus(int id, CancellationToken ct)
    {
        ...
        return Ok(ApiResponse.Ok($"Cat color {id} is now {(entity.IsActive ? "active" : "inactive")}."));
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        _logger.LogWarning("CatColor {Id} soft-deleted", id); return Ok(ApiResponse.Ok("Deleted."));
    }
}
