...
[ApiController]
[Route("api/activity/status-codes")]
[Authorize]
...
public sealed class StatusCodesController : ControllerBase
{
    ...
    public StatusCodesController(ApplicationDbContext db, ILogger<StatusCodesController> logger) { _db = db; _logger = logger; }
    ...
    {
        var q = _db.StatusCodes.AsNoTracking();
        ...
        return Ok(PagedResponse<StatusCode>.Ok(data, page, pageSize, total));
    }

    [HttpGet("{id:int}")]
    public async Task<IActionResult> GetById(int id, CancellationToken ct)
    {
        var e = await _db.StatusCodes.FindAsync([id], ct);
        return e is null ? NotFound(ApiResponse.Fail($"Status code {id} not found.")) : Ok(ApiResponse<StatusCode>.Ok(e));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CodeDescriptionDto dto, CancellationToken ct)
    {
        ...
        var entity = new StatusCode { Code = dto.Code, Description = dto.Description, IsActive = true, CreatedAt = DateTime.UtcNow };
        _db.StatusCodes.Add(entity); await _db.SaveChangesAsync(ct);
        return CreatedAtAction(nameof(GetById), new { id = entity.ScId }, ApiResponse<StatusCode>.Ok(entity, "Created."));
    }
    ...
    {
        if (!ModelState.IsValid) return BadRequest(ApiResponse.Fail(ModelState.Values.SelectMany(v => v.Errors.Select(e => e.ErrorMessage))));
        var entity = await _db.StatusCodes.FindAsync([id], ct);
        if (entity is null) return NotFound(ApiResponse.Fail($"Status code {id} not found."));
        entity.Code = dto.Code; entity.Description = dto.Description; await _db.SaveChangesAsync(ct);
        return Ok(ApiResponse<StatusCode>.Ok(entity, "Updated."));
    }
    ...
    public async Task<IActionResult> ToggleStatus(int id, CancellationToken ct)
    {
        ...
        return Ok(ApiResponse.Ok($"Status code {id} is now {(entity.IsActive ? "active" : "inactive")}."));
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        _logger.LogWarning("StatusCode {Id} soft-deleted", id); return Ok(ApiResponse.Ok("Deleted."));
    }
}
