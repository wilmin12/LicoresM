...
[ApiController]
[Route("api/activity/cat-sizes")]
[Authorize]
[Produces("application/json")]
public sealed class CatSizesController : ControllerBase
{
    private readonly ApplicationDbContext         _db;
    private readonly ILogger<CatSizesController> _logger;
    public CatSizesController(ApplicationDbContext db, ILogger<CatSizesController> logger) { _db = db; _logger = logger; }
    ...
    {
        var q = _db.CatSizes.AsNoTracking();
        ...
        return Ok(PagedResponse<CatSize>.Ok(data, page, pageSize, total));
    }

    [HttpGet("{id:int}")]
    public async Task<IActionResult> GetById(int id, CancellationToken ct)
    {
        var e = await _db.CatSizes.FindAsync([id], ct);
        return e is null ? NotFound(ApiResponse.Fail($"Cat size {id} not found.")) : Ok(ApiResponse<CatSize>.Ok(e));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CatSelDto dto, CancellationToken ct)
    {
        ...
        var entity = new CatSize { CatSel = dto.CatSel, IsActive = true, CreatedAt = DateTime.UtcNow };
        _db.CatSizes.Add(entity); await _db.SaveChangesAsync(ct);
        return CreatedAtAction(nameof(GetById), new { id = entity.CszId }, ApiResponse<CatSize>.Ok(entity, "Created."));
    }
    ...
    {
        if (!ModelState.IsValid) return BadRequest(ApiResponse.Fail(ModelState.Values.SelectMany(v => v.Errors.Select(e => e.ErrorMessage))));
        var entity = await _db.CatSizes.FindAsync([id], ct);
        if (entity is null) return NotFound(ApiResponse.Fail($"Cat size {id} not found."));
        entity.CatSel = dto.CatSel; await _db.SaveChangesAsync(ct);
        return Ok(ApiResponse<CatSize>.Ok(entity, "Updated."));
    }
    ...
    public async Task<IActionResult> ToggleStatus(int id, CancellationToken ct)
    {
        ...
        return Ok(ApiResponse.Ok($"Cat size {id} is now {(entity.IsActive ? "active" : "inactive")}."));
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        _logger.LogWarning("CatSize {Id} soft-deleted", id); return Ok(ApiResponse.Ok("Deleted."));
    }
}
