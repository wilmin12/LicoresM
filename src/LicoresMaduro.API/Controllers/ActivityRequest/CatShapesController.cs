...
[ApiController]
[Route("api/activity/cat-shapes")]
[Authorize]
[Produces("application/json")]
public sealed class CatShapesController : ControllerBase
{
    private readonly ApplicationDbContext          _db;
    private readonly ILogger<CatShapesController> _logger;
    public CatShapesController(ApplicationDbContext db, ILogger<CatShapesController> logger) { _db = db; _logger = logger; }
    ...
    {
        var q = _db.CatShapes.AsNoTracking();
        ...
        return Ok(PagedResponse<CatShape>.Ok(data, page, pageSize, total));
    }

    [HttpGet("{id:int}")]
    public async Task<IActionResult> GetById(int id, CancellationToken ct)
    {
        var e = await _db.CatShapes.FindAsync([id], ct);
        return e is null ? NotFound(ApiResponse.Fail($"Cat shape {id} not found.")) : Ok(ApiResponse<CatShape>.Ok(e));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CatSelDto dto, CancellationToken ct)
    {
        ...
        var entity = new CatShape { CatSel = dto.CatSel, IsActive = true, CreatedAt = DateTime.UtcNow };
        _db.CatShapes.Add(entity); await _db.SaveChangesAsync(ct);
        return CreatedAtAction(nameof(GetById), new { id = entity.Cs2Id }, ApiResponse<CatShape>.Ok(entity, "Created."));
    }
    ...
    {
        if (!ModelState.IsValid) return BadRequest(ApiResponse.Fail(ModelState.Values.SelectMany(v => v.Errors.Select(e => e.ErrorMessage))));
        var entity = await _db.CatShapes.FindAsync([id], ct);
        if (entity is null) return NotFound(ApiResponse.Fail($"Cat shape {id} not found."));
        entity.CatSel = dto.CatSel; await _db.SaveChangesAsync(ct);
        return Ok(ApiResponse<CatShape>.Ok(entity, "Updated."));
    }
    ...
    public async Task<IActionResult> ToggleStatus(int id, CancellationToken ct)
    {
        ...
        return Ok(ApiResponse.Ok($"Cat shape {id} is now {(entity.IsActive ? "active" : "inactive")}."));
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        _logger.LogWarning("CatShape {Id} soft-deleted", id); return Ok(ApiResponse.Ok("Deleted."));
    }
}
