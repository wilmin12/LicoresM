...
[ApiController]
[Route("api/activity/cat-gender")]
[Authorize]
[Produces("application/json")]
public sealed class CatGenderController : ControllerBase
{
    private readonly ApplicationDbContext           _db;
    private readonly ILogger<CatGenderController> _logger;
    public CatGenderController(ApplicationDbContext db, ILogger<CatGenderController> logger) { _db = db; _logger = logger; }
    ...
    {
        var q = _db.CatGenders.AsNoTracking();
        ...
        return Ok(PagedResponse<CatGender>.Ok(data, page, pageSize, total));
    }

    [HttpGet("{id:int}")]
    public async Task<IActionResult> GetById(int id, CancellationToken ct)
    {
        var e = await _db.CatGenders.FindAsync([id], ct);
        return e is null ? NotFound(ApiResponse.Fail($"Cat gender {id} not found.")) : Ok(ApiResponse<CatGender>.Ok(e));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CatSelDto dto, CancellationToken ct)
    {
        ...
        var entity = new CatGender { CatSel = dto.CatSel, IsActive = true, CreatedAt = DateTime.UtcNow };
        _db.CatGenders.Add(entity); await _db.SaveChangesAsync(ct);
        return CreatedAtAction(nameof(GetById), new { id = entity.CgId }, ApiResponse<CatGender>.Ok(entity, "Created."));
    }
    ...
    {
        if (!ModelState.IsValid) return BadRequest(ApiResponse.Fail(ModelState.Values.SelectMany(v => v.Errors.Select(e => e.ErrorMessage))));
        var entity = await _db.CatGenders.FindAsync([id], ct);
        if (entity is null) return NotFound(ApiResponse.Fail($"Cat gender {id} not found."));
        entity.CatSel = dto.CatSel; await _db.SaveChangesAsync(ct);
        return Ok(ApiResponse<CatGender>.Ok(entity, "Updated."));
    }
    ...
    public async Task<IActionResult> ToggleStatus(int id, CancellationToken ct)
    {
        ...
        return Ok(ApiResponse.Ok($"Cat gender {id} is now {(entity.IsActive ? "active" : "inactive")}."));
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        _logger.LogWarning("CatGender {Id} soft-deleted", id); return Ok(ApiResponse.Ok("Deleted."));
    }
}
