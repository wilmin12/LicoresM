...
[ApiController]
[Route("api/activity/pos-category")]
[Authorize]
[Produces("application/json")]
public sealed class PosCategoryController : ControllerBase
{
    private readonly ApplicationDbContext             _db;
    private readonly ILogger<PosCategoryController> _logger;
    public PosCategoryController(ApplicationDbContext db, ILogger<PosCategoryController> logger) { _db = db; _logger = logger; }
    ...
    {
        var q = _db.PosCategories.AsNoTracking();
        ...
        return Ok(PagedResponse<PosCategory>.Ok(data, page, pageSize, total));
    }

    [HttpGet("{id:int}")]
    public async Task<IActionResult> GetById(int id, CancellationToken ct)
    {
        var e = await _db.PosCategories.FindAsync([id], ct);
        return e is null ? NotFound(ApiResponse.Fail($"POS category {id} not found.")) : Ok(ApiResponse<PosCategory>.Ok(e));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CodeDescriptionDto dto, CancellationToken ct)
    {
        ...
        var entity = new PosCategory { Code = dto.Code, Description = dto.Description, IsActive = true, CreatedAt = DateTime.UtcNow };
        _db.PosCategories.Add(entity); await _db.SaveChangesAsync(ct);
        return CreatedAtAction(nameof(GetById), new { id = entity.PcId }, ApiResponse<PosCategory>.Ok(entity, "Created."));
    }
    ...
    {
        if (!ModelState.IsValid) return BadRequest(ApiResponse.Fail(ModelState.Values.SelectMany(v => v.Errors.Select(e => e.ErrorMessage))));
        var entity = await _db.PosCategories.FindAsync([id], ct);
        if (entity is null) return NotFound(ApiResponse.Fail($"POS category {id} not found."));
        entity.Code = dto.Code; entity.Description = dto.Description; await _db.SaveChangesAsync(ct);
        return Ok(ApiResponse<PosCategory>.Ok(entity, "Updated."));
    }
    ...
    public async Task<IActionResult> ToggleStatus(int id, CancellationToken ct)
    {
        ...
        return Ok(ApiResponse.Ok($"POS category {id} is now {(entity.IsActive ? "active" : "inactive")}."));
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        _logger.LogWarning("PosCategory {Id} soft-deleted", id); return Ok(ApiResponse.Ok("Deleted."));
    }
}
