...
[ApiController]
[Route("api/activity/denial-reasons")]
[Authorize]
[Produces("application/json")]
public sealed class DenialReasonsController : ControllerBase
{
    private readonly ApplicationDbContext             _db;
    private readonly ILogger<DenialReasonsController> _logger;
    public DenialReasonsController(ApplicationDbContext db, ILogger<DenialReasonsController> logger) { _db = db; _logger = logger; }
    ...
    {
        var q = _db.DenialReasons.AsNoTracking();
        ...
        return Ok(PagedResponse<DenialReason>.Ok(data, page, pageSize, total));
    }

    [HttpGet("{id:int}")]
    public async Task<IActionResult> GetById(int id, CancellationToken ct)
    {
        var e = await _db.DenialReasons.FindAsync([id], ct);
        return e is null ? NotFound(ApiResponse.Fail($"Denial reason {id} not found.")) : Ok(ApiResponse<DenialReason>.Ok(e));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CodeDescriptionDto dto, CancellationToken ct)
    {
        ...
        var entity = new DenialReason { Code = dto.Code, Description = dto.Description, IsActive = true, CreatedAt = DateTime.UtcNow };
        _db.DenialReasons.Add(entity); await _db.SaveChangesAsync(ct);
        return CreatedAtAction(nameof(GetById), new { id = entity.DrId }, ApiResponse<DenialReason>.Ok(entity, "Created."));
    }
    ...
    {
        if (!ModelState.IsValid) return BadRequest(ApiResponse.Fail(ModelState.Values.SelectMany(v => v.Errors.Select(e => e.ErrorMessage))));
        var entity = await _db.DenialReasons.FindAsync([id], ct);
        if (entity is null) return NotFound(ApiResponse.Fail($"Denial reason {id} not found."));
        entity.Code = dto.Code; entity.Description = dto.Description; await _db.SaveChangesAsync(ct);
        return Ok(ApiResponse<DenialReason>.Ok(entity, "Updated."));
    }
    ...
    public async Task<IActionResult> ToggleStatus(int id, CancellationToken ct)
    {
        ...
        return Ok(ApiResponse.Ok($"Denial reason {id} is now {(entity.IsActive ? "active" : "inactive")}."));
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        _logger.LogWarning("DenialReason {Id} soft-deleted", id); return Ok(ApiResponse.Ok("Deleted."));
    }
}
