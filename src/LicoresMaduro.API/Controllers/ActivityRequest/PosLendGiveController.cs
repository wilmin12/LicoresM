...
[ApiController]
[Route("api/activity/pos-lend-give")]
[Authorize]
[Produces("application/json")]
public sealed class PosLendGiveController : ControllerBase
{
    private readonly ApplicationDbContext             _db;
    private readonly ILogger<PosLendGiveController> _logger;
    public PosLendGiveController(ApplicationDbContext db, ILogger<PosLendGiveController> logger) { _db = db; _logger = logger; }
    ...
    {
        var q = _db.PosLendGives.AsNoTracking();
        ...
        return Ok(PagedResponse<PosLendGive>.Ok(data, page, pageSize, total));
    }

    [HttpGet("{id:int}")]
    public async Task<IActionResult> GetById(int id, CancellationToken ct)
    {
        var e = await _db.PosLendGives.FindAsync([id], ct);
        return e is null ? NotFound(ApiResponse.Fail($"POS lend/give {id} not found.")) : Ok(ApiResponse<PosLendGive>.Ok(e));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CodeDescriptionDto dto, CancellationToken ct)
    {
        ...
        var entity = new PosLendGive { Code = dto.Code, Description = dto.Description, IsActive = true, CreatedAt = DateTime.UtcNow };
        _db.PosLendGives.Add(entity); await _db.SaveChangesAsync(ct);
        return CreatedAtAction(nameof(GetById), new { id = entity.PlgId }, ApiResponse<PosLendGive>.Ok(entity, "Created."));
    }
    ...
    {
        if (!ModelState.IsValid) return BadRequest(ApiResponse.Fail(ModelState.Values.SelectMany(v => v.Errors.Select(e => e.ErrorMessage))));
        var entity = await _db.PosLendGives.FindAsync([id], ct);
        if (entity is null) return NotFound(ApiResponse.Fail($"POS lend/give {id} not found."));
        entity.Code = dto.Code; entity.Description = dto.Description; await _db.SaveChangesAsync(ct);
        return Ok(ApiResponse<PosLendGive>.Ok(entity, "Updated."));
    }
    ...
    public async Task<IActionResult> ToggleStatus(int id, CancellationToken ct)
    {
        ...
        return Ok(ApiResponse.Ok($"POS lend/give {id} is now {(entity.IsActive ? "active" : "inactive")}."));
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken ct)
    {
        ...
        _logger.LogWarning("PosLendGive {Id} soft-deleted", id); return Ok(ApiResponse.Ok("Deleted."));
    }
}
