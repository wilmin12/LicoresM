...
public sealed class VehicleTypeController : ControllerBase
{
    ...
    public VehicleTypeController(ApplicationDbContext db, IPermissionService permissions, ILogger<VehicleTypeController> logger)
    ...
    {
        if (!await _permissions.HasPermissionAsync(User, "AB_VEHICLE_TYPE", "WRITE", ct))
            return Forbid();
    ...
    {
        if (!await _permissions.HasPermissionAsync(User, "AB_VEHICLE_TYPE", "EDIT", ct))
            return Forbid();
        if (!ModelState.IsValid)
            return BadRequest(ApiResponse.Fail(ModelState.Values.SelectMany(v => v.Errors.Select(e => e.ErrorMessage))));
    ...
    {
        if (!await _permissions.HasPermissionAsync(User, "AB_VEHICLE_TYPE", "DELETE", ct))
            return Forbid();
        var entity = await _db.VehicleTypes.FindAsync([id], ct);
        if (entity is null) return NotFound(ApiResponse.Fail($"Vehicle type {id} not found."));
