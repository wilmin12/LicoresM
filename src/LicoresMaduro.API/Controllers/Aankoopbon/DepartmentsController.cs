...
public sealed class DepartmentsController : ControllerBase
{
    ...
    public DepartmentsController(ApplicationDbContext db, IPermissionService permissions, ILogger<DepartmentsController> logger)
    ...
    {
        if (!await _permissions.HasPermissionAsync(User, "AB_DEPARTMENTS", "WRITE", ct))
            return Forbid();
    ...
    {
        if (!await _permissions.HasPermissionAsync(User, "AB_DEPARTMENTS", "EDIT", ct))
            return Forbid();
        if (!ModelState.IsValid)
            return BadRequest(ApiResponse.Fail(ModelState.Values.SelectMany(v => v.Errors.Select(e => e.ErrorMessage))));
    ...
    {
        if (!await _permissions.HasPermissionAsync(User, "AB_DEPARTMENTS", "DELETE", ct))
            return Forbid();
        var entity = await _db.Departments.FindAsync([id], ct);
        if (entity is null) return NotFound(ApiResponse.Fail($"Department {id} not found."));
